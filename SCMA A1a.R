...
setwd('C:\\Users\\Asus\\OneDrive\\Desktop\\New folder (2)')
getwd()
...
uttnew$Meals_At_Home <- impute_with_mean(uttnew$Meals_At_Home)
...
for (col in outlier_columns) {
  uttnew <- remove_outliers(uttnew, col)
}

# Summarize consumption
uttnew$total_consumption <- rowSums(uttnew[, c("ricepds_v", "Wheatpds_q", "chicken_q", "pulsep_q", "wheatos_q")], na.rm = TRUE)
...
district_mapping <- c("05" = "Dehradun", "13" = "Hardwar", "06" = "Garhwal")
sector_mapping <- c("2" = "URBAN", "1" = "RURAL")

uttnew$District <- as.character(uttnew$District)
uttnew$Sector <- as.character(uttnew$Sector)
uttnew$District <- ifelse(uttnew$District %in% names(district_mapping), district_mapping[uttnew$District], uttnew$District)
uttnew$Sector <- ifelse(uttnew$Sector %in% names(sector_mapping), sector_mapping[uttnew$Sector], uttnew$Sector)
